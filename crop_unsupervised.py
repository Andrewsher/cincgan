...
def parse_args():
    ...
    # config
    parser.add_argument('--hr-path', default='/data/data/DIV2K/unsupervised/hr', type=str)
    parser.add_argument('--lr-path', default='/data/data/DIV2K/unsupervised/lr', type=str)
    parser.add_argument('--target-dir', default='/data/data/DIV2K/unsupervised/train_dataset', type=str)
...
def crop(args):
    ...
    for image_name in tqdm(os.listdir(args.lr_path)):
        ...
        # crop
        for cx in range(0, image_lr.size[0] - (args.crop_size // args.scale), args.crop_step // args.scale):
            for cy in range(0, image_lr.size[1] - (args.crop_size // args.scale), args.crop_step // args.scale):
                current_image_name = '{}_{}_{}.png'.format(image_name.split('.')[0], str(cx), str(cy))
                # lr
